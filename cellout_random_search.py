...
def load_clean_set(root, epoch):
    ...
    for i in range(1, 4):
        if i == 1:
            dir = root + '08/trial3/' + 'cellout_clean1.json'
        else:
            dir = root + '08/trial3/' + f'cellout_clean08_{i}.json'
...
def load_noisy_set(root):
    ...
    dir = root + '08/trial3/' + 'cellout_noisy08_1.json'
    with open(dir, "r") as f:
        arch_pairs = json.load(f)
    for arch_pair in arch_pairs:
        arch = arch_pair['arch']
